...
void detectObjects(cv::Mat& img, std::vector<BoundingBox>& bBoxes, float confThreshold, float nmsThreshold, 
                   std::string basePath, std::string classesFile, std::string modelConfiguration, std::string modelWeights, cv::Mat &visImg, bool bVis)
{
    ...
    if(bVis) {
        ...
        for(auto it=bBoxes.begin(); it!=bBoxes.end(); ++it) {
            ...
            cv::putText(visImg, "#"+to_string(it->boxID), cv::Point(left+width-30, top+height-10), cv::FONT_ITALIC, 0.75, cv::Scalar(0,0,255),1);
